...
import os, sys, json, re, tempfile, wave, urllib.request, base64, uuid, random
...
@app.post("/api/analyze-interpretation")
...
@app.post("/api/analyze-voiceover")
...
@app.post("/api/analyze-video-terms")
...
@app.post("/api/analyze-video-structure")
...
@app.post("/api/opi/prefetch-next")
...
async def opi_end_call(session_id: str = Form(...)):
    ...
    if not sess:
        ...
        return JSONResponse({
            ...
            "terminology": 0, "fluency": 0, "professional_protocol": 0,
    ...
    result = extract_json(ask_claude(f"""You are a certified AIIC-standard OPI evaluator. Assess this interpreting session rigorously — do not inflate scores.
...
• accuracy (25%): Exact meaning conveyed — no distortions, insertions, or embellishments. Every factual element preserved.
...
• completeness (20%): ALL elements of the source present in the rendition. Omissions and condensations penalized proportionally.
...
overall_score = 0.25×accuracy + 0.20×fusha_compliance + 0.20×completeness + 0.15×terminology + 0.10×fluency + 0.10×professional_protocol

{intro_block}{video_block}
TURN PAIRS (assess each one — up to turn {min(len(ordered_pairs), 8)}):
{pairs_numbered}
...
{{
  ...
  "grade": "Excellent (≥88)|Good (75-87)|Satisfactory (60-74)|Needs Work (<60)",
...
Grade thresholds: Excellent ≥88, Good 75-87, Satisfactory 60-74, Needs Work <60.
Brevity is mandatory: note ≤20 words, ideal_rendition ≤25 words, summary exactly 2 sentences.""", 2000))
...
#   • No response latency KPI (speaker controls natural pauses)
...
#   overall = 0.25×accuracy + 0.20×fusha_compliance + 0.25×completeness
#           + 0.15×terminology + 0.10×fluency + 0.05×professional_protocol
# ──────────────────────────────────────────────────────────────────────────────
...
async def ci_new_session(
    ...
    mode:         str = Form("consecutive"),   # "consecutive" | "simultaneous" | "chuchotage" | "escort" | "sight"
    ...
    try:
        ...
        wpm_target = {1: 50.0, 2: 62.0, 3: 75.0, 4: 88.0}.get(pace, 62.0) if is_sight else 0.0
...
async def ci_end_session(session_id: str = Form(...)):
    ...
    if not sess:
        return JSONResponse({
            ...
            "professional_protocol": 0, "memory_accuracy": 0, "segment_handling": 0,
    ...
    if is_si or is_chuchotage:
        ...
        if is_chuchotage:
            ...
            second_kpi_desc = "Combined volume discipline (whispering at a controlled, listener-appropriate level) and noise resilience (tracking the source despite ambient noise/no audio isolation)."
...
• accuracy (30%): Exact meaning preserved under live cognitive load — no distortions, embellishments, or additions not in source. Weighted higher than CI since there is no recall buffer.
...
• completeness (20%): Elements of source preserved despite continuous-speech pressure. Strategic, minor condensing of redundant content is acceptable; unintentional drops of substantive content are not.
• terminology (15%): Correct domain-specific terms for {sess['field']} produced in real time.
• fluency (15%): Output fluency UNDER COGNITIVE LOAD — smoothness, lack of false starts/self-correction churn, ability to keep pace with continuous source. Weighted higher than CI since there's no stop-and-recover.
...
overall_score = 0.30×accuracy + 0.15×fusha_compliance + 0.20×completeness + 0.15×terminology + 0.15×fluency + 0.05×professional_protocol
...
{{
  ...
  "segment_handling": <live_fluency 0-100 — see SI-specific KPIs above>,
  "grade": "Excellent (≥88)|Good (75-87)|Satisfactory (60-74)|Needs Work (<60)",
...
Grade: Excellent ≥88, Good 75-87, Satisfactory 60-74, Needs Work <60.""", 2500))
        ...
        if is_chuchotage:
            ...
            result["kpi_labels"] = {"memory_accuracy": "Décalage Control", "segment_handling": "Volume & Noise Discipline"}
        else:
...
• fusha_compliance (10%): For Arabic output, prefer clear standard register; light informal register is acceptable in this mode and should NOT be penalized as "dialect" unless it becomes genuinely unclear or unprofessional.
• completeness (20%): Substantive content carried across in both directions — minor conversational small talk may be condensed without penalty.
...
• fluency (20%): Natural, conversational delivery that doesn't feel stilted or overly formal for the setting — turn-taking handled smoothly in both directions.
...
overall_score = 0.25×accuracy + 0.10×fusha_compliance + 0.20×completeness + 0.10×terminology + 0.20×fluency + 0.15×professional_protocol
...
Grade: Excellent ≥88, Good 75-87, Satisfactory 60-74, Needs Work <60.""", 2500))
        ...
        wpm_line = (
            f"Measured average WPM throughput: {wpm_avg} against a target of ~{wpm_target} WPM (protocol benchmark ~60-65 WPM sustained, read-while-speak load). "
...
• fusha_compliance (15%): For Arabic output, Modern Standard Arabic / Fusha only, matching the written register. N/A → 100 for non-Arabic output.
• completeness (20%): ALL elements of the written document rendered — numbers, dates, names, clause structure. Omissions are penalized heavily, as with a citable document.
• terminology (15%): Correct domain/register-specific terms and document conventions (e.g. legal/contractual phrasing, form-field structure) reproduced faithfully.
...
• wpm_throughput: Sustained reading-while-speaking pace against the ~60-65 WPM protocol benchmark (use the measured value above as ground truth when provided) — distinct from spoken-source Pace, since this reflects read-aloud throughput under cognitive load.
...
overall_score = 0.25×accuracy + 0.15×fusha_compliance + 0.20×completeness + 0.15×terminology + 0.15×fluency + 0.10×professional_protocol
...
• fusha_compliance (20%): Arabic output in Modern Standard Arabic / Fusha only. Penalize dialect, slang, code-switching. N/A → 100 for non-Arabic output.
• completeness (25%): ALL elements of source present in rendition. In CI this is critical — longer segments mean more to recall. Omissions penalized heavily.
...
overall_score = 0.25×accuracy + 0.20×fusha_compliance + 0.25×completeness + 0.15×terminology + 0.10×fluency + 0.05×professional_protocol
